...
**Status.** Stable. Current release: **v1.6.7** runtime paired with **spec v4.14**. See [CHANGELOG.md](./CHANGELOG.md) for the release history. v1.x releases follow a frozen-public-surface contract (see [CONTRIBUTING.md](./CONTRIBUTING.md) for the v1.x semver policy: patch additive within frozen surface, minor additive only, major requires a new trilateral cross-review session). v1.0 was cut on 2026-04-25 after a 10-session field-use validation gate per operator directive 2026-04-24, ratified by trilateral final approval session `fca13b80`.

The version history at a glance:

| Release | Spec | Scope |
|---|---|---|
| **`v1.6.7`** | v4.14 | **Audit-closure hardening pass (12 of 15 priorities from the v1.6.6 internal audit; 3 deferred to a refactor-scoped release).** Adds MCP `tools/list` schema caps (`maxLength`/`maxItems`/`pattern` on `task`/`prompt`/`question`/`context`/`label`/`content`/`session_id` etc.) so oversized inputs are rejected at the schema layer before the parser/spawn/persistence layers touch them. Hardens `atomicWriteFile` with retry/backoff for transient Windows `EPERM`/`EACCES`/`EBUSY` and self-cleans orphan `*.tmp.*` files via a new boot sweep. Adds a companion boot sweep that clears stale `meta.in_flight` when the holder PID is dead or `last_heartbeat > 2 × HEARTBEAT_INTERVAL_MS`. Caps the structured `<cross_review_status>` payload at 64 KiB and `<cross_review_peer_model>` at 8 KiB **before** `JSON.parse`, blocking OOM via a hostile/buggy peer. Extends `convergence_scope` with `degraded_trilateral`/`degraded_quadrilateral` so spawn-rejected peers are reported as degraded scope rather than masked as the full-cohort label (cross-review-v2 R1 retired the initially-proposed `silent_degraded_*` prefix in favor of widening the existing `degraded_*` vocabulary). Replaces the naive argv[0] parser in the orphan sweep with a shlex-style state machine (defense-in-depth still backstops via `findOrphans`/`ancestorPidSet`). Adds `CROSS_REVIEW_STATE_DIR` env override so non-`$HOME` deployments work without code edits. Centralizes `normalizeOutcomeReason` in the store so the same null/empty/whitespace collapse applies across all callers. Documents the single-user trusted-host threat model + multi-host limitations in `SECURITY.md` and `CONTRIBUTING.md`. **Deferred to a future release:** P2.5 decomposition of the `ask_peer`/`ask_peers` handlers (~355 lines today), P2.7 centralized zod/ajv validation at the MCP boundary, P3.9 split of the 8 054-line `functional-smoke.js` into per-area files. |
| **`v1.6.6`** | v4.14 | **Audit correctness patch.** Fixes probe overflow diagnostics so stdout-overflow probes preserve the stdout tail, hardens stale-session scanning so invalid UUID-length filesystem noise cannot break `session_sweep`, and detaches probe/peer stream listeners on spawn-error paths for consistency with timeout/overflow cleanup. |
